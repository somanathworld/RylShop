...
export default function AddToCartBtnComponent({ product, variant }) {
    ...
    useEffect(()=>{
        async function fetchPinCodes(){
            let resp = await fetch("http://localhost:3000/api/pinservice");
            let data = await resp.json();
            setAvailablePins(data.pincodes);
        }
        fetchPinCodes();
    },[pin])

    function checkAvailability(event) {
        if (Object.keys(availablePins).includes(pin)) {
            setIsAvailable(true);
    ...
    return (<>
        <div className="flex">
            ...
            <button onClick={() => buyNow(productId, 1, product.price, product.title, product.images[0], variant.size, variant.color)} className="flex ml-auto text-white bg-blue-500 text-sm md:text-base py-2 px-2 border-0 md:py-2 md:px-4 focus:outline-none hover:bg-blue-600 rounded">Buy Now</button>
            <button onClick={() => addToCart(productId, 1, product.price, product.title, product.images[0], variant.size, variant.color)} className="flex ml-auto text-white bg-blue-500 text-sm md:text-base py-2 px-2 border-0 md:py-2 md:px-4 focus:outline-none hover:bg-blue-600 rounded">AddTo Cart</button>
