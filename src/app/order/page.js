...
export default function OrderPage() {
    ...
    const [cart, addToCart, removeQtyFromCart, clearCart, subTotal] = useContext(ProductContext);

    return (
        <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">

            <div className="flex justify-start item-start space-y-2 flex-col">
                <h1 className="text-3xl text-grey-300 lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">Order #13432</h1>
                <p className="text-base text-grey-300 font-medium leading-6 text-gray-600">21st Mart 2021 at 10:34 PM</p>
            </div>
                        ...
                        {Object.keys(cart).map(item => (
                            <div key = {item} className="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
                                <div className="pb-4 md:pb-8 w-full md:w-40">
                                    <img className="w-full hidden md:block" src="https://m.media-amazon.com/images/I/61zqqJJtrqL._AC_UL320_.jpg" alt="dress" />
                                    <img className="w-full md:hidden" src="https://m.media-amazon.com/images/I/61zqqJJtrqL._AC_UL320_.jpg" alt="dress" />
                                </div>
                                <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                                    <div className="w-full flex flex-col justify-start items-start space-y-8">
                                        <h3 className="text-xl dark:text-white xl:text-2xl font-semibold leading-6 text-gray-800">{cart[item].name}</h3>
                                        <div className="flex justify-start items-start flex-col space-y-2">
                                            <p className="text-sm dark:text-white leading-none text-gray-800"><span className="dark:text-gray-400 text-gray-300">Style: </span> {cart[item].name}</p>
                                            <p className="text-sm dark:text-white leading-none text-gray-800"><span className="dark:text-gray-400 text-gray-300">Size: </span> {cart[item].size}</p>
                                            <p className="text-sm dark:text-white leading-none text-gray-800"><span className="dark:text-gray-400 text-gray-300">Color: </span> {cart[item].variant}</p>
                                        </div>
                                    </div>
                                    <div className="flex justify-between space-x-8 items-start w-full">
                                        ...
                                        <p className="text-base dark:text-white xl:text-lg leading-6 text-gray-800">{cart[item].qty}</p>
